...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if not self.current_map.collision(self.next_x, self.next_y):
            self.rect.x += int(self.speedx)
            self.rect.y += int(self.speedy)

        # Проверка на нажаите триггера
        if self.current_map.trigger(self.next_x, self.next_y) > 1:
            id = self.current_map.trigger(self.next_x, self.next_y)
            if self.current_map == map.map1:
                if id == 2:
                    self.current_map = map.map2

                if id == 3:
                    self.current_map = map.map3
            if self.current_map == map.map2:
                pass
